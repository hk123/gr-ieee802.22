...
static const int MIN_GAP = 15000;
static const int MAX_SAMPLES = 540 * 2560;
		...
		for(i = 0; i < ninput; i++) {
			if(in_cor[i] > d_threshold) {
				if(d_plateau < MIN_PLATEAU) {
					...
					d_state = COPY;
					d_copied = 0;
					d_freq_offset = arg(in_abs[i]) / 512;
					d_plateau = 0;
		...
		while( o < ninput && o < noutput && d_copied < MAX_SAMPLES) {
			if(in_cor[o] > d_threshold) {
				...
				// there's another frame
					...
					dout << "d_copied: " << d_copied << std::endl;
					d_copied = 0;
					d_plateau = 0;
					d_freq_offset = arg(in_abs[o]) / 512;
					insert_tag(nitems_written(0) + o);
					dout << "SHORT Frame!" << std::endl;
					dout <<  "nitems_written(0)+o: " << nitems_written(0)+o << std::endl;
					break;
				}
